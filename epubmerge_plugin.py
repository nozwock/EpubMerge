...
class EpubMergePlugin(InterfaceAction):
    ...
    def initialization_complete(self):
        # setup menu.
        self.merge_action = self.create_menu_item_ex(
            self.menu,
            _("&Merge Epubs"),
        ...
        self.unmerge_action = self.create_menu_item_ex(
            self.menu,
            _("&UnMerge Epubs"),
            image="images/unmerge.png",
    ...
    def check_macmenuhack(self):
        try:
            ...
            file_path = os.path.join(
                calibre_config_dir, *("plugins/fanficfare_macmenuhack.txt".split("/"))
            )
        ...
        for book_id in self.gui.library_view.get_selected_ids():
            ...
            if db.has_format(book_id, "EPUB", index_is_id=True):
                ...
                d = error_dialog(
                    ...
                    + "<br>"
                    + _(
                        "To UnMerge the source must be Epub(s) created by EpubMerge with Keep UnMerge Metadata enabled."
                    ),
            ...
            for formats in db.find_books_in_directory(tdir, False):
                ...
                if identicalbooks:
                    if len(identicalbooks) == 1:
                        text = (
                            ...
                            + "<br>"
                            + _(
                                "You already have a book <i>%s</i> by <i>%s</i>.  You may Add a new book of the same title, Overwrite the Epub in the existing book, or Discard this Epub."
                            )
                        ...
                        text = (
                            ...
                            + "<br>"
                            + _(
                                "You already have more than one book <i>%s</i> by <i>%s</i>.  You may Add a new book of the same title, or Discard this Epub."
                            )
                            % (mi.title, ", ".join(mi.authors))
                        )
                        over = False
                    ...
                    if applyall and (over or state in ("add", "discard")):
                        ...
                        d = AddOverDiscardDialog(
                            self.gui, self.qaction.icon(), text, over=over
                        )
                        d.exec_()
                        state = d.state
                ...
                db.add_format_with_hooks(book_id, "EPUB", formats[0], index_is_id=True)
    ...
    def plugin_button(self):
        ...
        if len(self.gui.library_view.get_selected_ids()) < 2:
            ...
            book_list = [
                self._convert_id_to_book(x, good=False)
                for x in self.gui.library_view.get_selected_ids()
            ]
            ...
            LoopProgressDialog(
                ...
                book_list,
                partial(
                    self._populate_book_from_calibre_id,
                    db=self.gui.current_db,
                    tdir=tdir,
                ),
    ...
    def _start_merge(self, book_list, tdir=None):
        ...
        if len(bad_list) > 0:
            ...
            if prefs["includecomments"]:
                ...
                mi.comments += (
                    '<div class="mergedbook">'
                    + '<hr></div><div class="mergedbook">'.join(
                        [bookcomments(x) for x in book_list]
                    )
                    + "</div>"
                )
            else:
            ...
            for col, action in six.iteritems(prefs["custom_cols"]):
                ...
                if action not in permitted_values[coldef["datatype"]]:
                    logger.debug(
                        "%s not a valid column type for %s, skipping." % (col, action)
                    )
                    continue
                ...
                if action == "union":
                    if not coldef["is_multiple"]:
                        ...
                        for bid in idslist:
                            try:
                                value = value.union(
                                    db.get_custom(bid, label=label, index_is_id=True)
                                )
                                found = True
                            except:
                                # if not set, it's None and fails.
...
You are merging %s EPUBs totaling %s.""")
            ...
            mergedepub = PersistentTemporaryFile(
                prefix="output_", suffix=".epub", dir=tdir
            )
            epubstomerge = [x["epub"] for x in book_list]
            epubtitles = {}
            for x in book_list:
                # save titles indexed by epub for reporting from BG
                epubtitles[x["epub"]] = _("%s by %s") % (
                    x["title"],
                    " & ".join(x["authors"]),
                )
            ...
            if mi.has_cover:
                ...
                coverjpgpath = os.path.join(
                    db.library_path, db.path(book_id, index_is_id=True), "cover.jpg"
                )

            func = "arbitrary_n"
            cpus = self.gui.job_manager.server.pool_size
            args = [
                "calibre_plugins.epubmerge.jobs",
                "do_merge_bg",
                (
                    {
                        "book_id": book_id,
            ...
            job = self.gui.job_manager.run_job(
                self.Dispatcher(self.merge_done), func, args=args, description=desc
            )
    ...
    def merge_done(self, job):
        ...
        self.t = time.time()
        db.add_format_with_hooks(book_id, "EPUB", outputepubfn, index_is_id=True)
        ...
        self.gui.status_bar.show_message(
            _("Finished merging %s EPUBs.") % book_count, 3000
        )
        ...
        confirm(
            "\n"
            + _(
                """EpubMerge has finished. The new EPUB has been added to the book previously created."""
            ),
    ...
    def _populate_book_from_calibre_id(self, book, db=None, tdir=None):
        ...
        if db.has_format(mi.id, "EPUB", index_is_id=True):
            if prefs["keepmeta"]:
                ...
                # data.
                tmp = PersistentTemporaryFile(
                    prefix="input_%s_" % mi.id, suffix=".epub", dir=tdir
                )
            ...
            book["good"] = False
            book["error"] = _("%s by %s doesn't have an EPUB.") % (
                mi.title,
                ", ".join(mi.authors),
            )
