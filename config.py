...
def set_library_config(library_config):
    get_gui().current_db.prefs.set_namespaced(
        PREFS_NAMESPACE, PREFS_KEY_SETTINGS, library_config
    )
...
def get_library_config():
    ...
    if library_config is None:
        ...
        library_config = db.prefs.get_namespaced(
            PREFS_NAMESPACE, PREFS_KEY_SETTINGS, copy.deepcopy(default_prefs)
        )
...
are unchecked, there will be no Table of Contents in merged books.""")
        self.titlenavpoints = QCheckBox(
            _("Insert Table of Contents entry for each title?"), self
        )
...
it's existing TOC nested underneath it.""")
        ...
        self.originalnavpoints = QCheckBox(
            _("Copy Table of Contents entries from each title?"), self
        )
        self.originalnavpoints.setToolTip(
            _("""If set, the original TOC entries will be included the new epub.""")
            + "\n"
            + no_toc_warning
        )
        ...
        def f():
            if (
                not self.originalnavpoints.isChecked()
                and not self.titlenavpoints.isChecked()
            ):
                confirm(
                    "<br>" + no_toc_warning,  # force HTML to get auto wrap.
        ...
        ## keepsingletocs
        self.removesingletocs = QCheckBox(
            _("Skip when there is only one TOC entry?"), self
        )
        self.removesingletocs.setToolTip(
            _("""If set, the original TOC entries for each book will only be included
...
columns.  Leave off if you plan to distribute the epub to others.""")
        ...
        self.restore_selection.setToolTip(
            _("""If set, the plugin will select the same books again after starting
...
For people who don't like the word Anthology.""")
        ...
        label = QLabel(
            _(
                "These controls aren't plugin settings as such, but convenience buttons for setting Keyboard shortcuts and getting all the EpubMerge confirmation dialogs back again."
            )
        )
        label.setWordWrap(True)
        self.l.addWidget(label)
    ...
    def reset_dialogs(self):
        for key in dynamic.keys():
            if (
                key.startswith("epubmerge_")
                and key.endswith("_again")
...
class ColumnsTab(QWidget):
    def __init__(self, parent_dialog, plugin_action):
        ...
        self.firstseries.setToolTip(
            _(
                """If set, the Series name and index from the first book will be set on the merged book."""
            )
        )
        ...
        label = QLabel(
            _(
                "If you have custom columns defined, they will be listed below.  Choose how you would like these columns handled."
            )
        )
        label.setWordWrap(True)
        self.l.addWidget(label)
        ...
        for key, column in sorted(custom_columns.items(), key=lambda x: x[1]["name"]):
            if column["datatype"] in permitted_values:
                ...
                label.setToolTip(
                    _("Set this %s column on new merged books...") % column["datatype"]
                )
                ...
                if key in prefs["custom_cols"]:
                    dropdown.setCurrentIndex(
                        dropdown.findData(prefs["custom_cols"][key])
                    )
